...
export default function DashboardScreen({ navigation }: any) {
  ...
  const renderRecentEntries = () => (
    <View style={{ marginTop: 16 }}>
      <Text style={{ fontWeight: '700', color: '#001F3F', marginBottom: 8 }}>Recent Entries</Text>
      {entries.length === 0 ? (
        <Paragraph style={{ color: '#6B7280', marginTop: 12 }}>No entries yet. Add your first daily entry.</Paragraph>
      ) : (
        entries.slice(0, 8).map(item => (
          <Card key={item.id} style={{ marginBottom: 8, borderRadius: 8 }}>
            <Card.Content>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                ...
                <Paragraph style={{ fontWeight: '700', color: item.profit >= 0 ? '#4CAF50' : '#FF5252' }}>{formatCurrency(item.profit)}</Paragraph>
              </View>
              {item.notes ? <Paragraph style={{ marginTop: 6, color: '#6B7280' }}>{item.notes}</Paragraph> : null}
            </Card.Content>
          </Card>
        ))
      )}
